...
ブラウザ ─ / (Next.js, App Router)
            ...
            └─ FloatingTerminal (business) ─ Excel風グリーンテーマ
                  ...
                  └─ 裏面: Settings + Shell
                       ...
                       ▼
      ┌────────────────────────────────────────┐
      │ Next.js (:3000) + pty-server (:4097)   │  ← ネイティブ or Docker
      │   └─ opencode (TUI)                    │
      └────────────────────────────────────────┘
                       │ OpenAI 互換 (host networking / localhost)
                       ▼
      llama-server (:8080) ──▶ Gemma   ← 常にホスト側
      LionFrame (:3030, OIDC Provider, Docker)
```

ターミナルはシーン座標にアンカーされているため、ホワイトボードをパン／ズームするとそれに追従して移動・拡大縮小します。

`opencode` と `pty-server` を動かす実行環境は **ネイティブ** (`npm run dev:all`) と **Docker コンテナ** (`docker compose up`) の 2 択で、どちらでもブラウザから同じ `http://localhost:3000` で使えます。
...
- **フリップターミナル** — 表面は OpenCode、裏面はインタラクティブシェル + 設定パネル。ヘッダ右端のアイコンで回転アニメーション切替。裏面シェルで `next dev -p 3000` 等のサーバを直接起動可能。
...
LionFrame（Docker コンテナで `http://localhost:3030` で稼働）の管理 UI でこのアプリを OIDC クライアントとして登録し、以下を設定してください:
...
3. ブラウザで http://localhost:3000 を開く。未認証なら `/login` に誘導され、「LionFrame でログイン」で認証フローに入る（LionFrame は Docker コンテナで :3030 で稼働している想定）。
...
- ホスト側で LionFrame (:3030, Docker コンテナ) と llama-server (:8080) が稼働していること
...
初回は opencode CLI と npm 依存のビルドのため数分かかります。その後はブラウザで http://localhost:3000 を開くだけ（ネイティブ起動と同じ URL）。
...
- コンテナから LionFrame や llama.cpp に繋がらない → host networking が効いているか確認: `docker compose exec opencode-demo curl http://localhost:3030/api/oidc/.well-known/openid-configuration`
...
| `LIONFRAME_ISSUER` | — | LionFrame OIDC Provider の Issuer ベース URL（例: `http://localhost:3030/api/oidc`） |
| `LIONFRAME_CLIENT_ID` | — | LionFrame で登録したクライアント ID |
| `LIONFRAME_CLIENT_SECRET` | — | LionFrame で登録したクライアントシークレット |
| `NEXTAUTH_URL` | `http://localhost:3000` | RP（この opencode-demo）の公開 URL。`redirect_uri` 組み立てに使用 |
